...
import (
	...
	"github.com/luckymark84/gooxml/common"
	"github.com/luckymark84/gooxml/zippkg"

	"github.com/luckymark84/gooxml/schema/soo/dml"
	st "github.com/luckymark84/gooxml/schema/soo/ofc/sharedTypes"
	"github.com/luckymark84/gooxml/schema/soo/pkg/relationships"
...
func New() *Document {
	...
	//d.ContentTypes.AddOverride("/word/glossary/document.xml", "application/vnd.openxmlformats-officedocument.wordprocessingml.document.glossary+xml")
	//d.ContentTypes.AddOverride("/word/glossary/styles.xml", "application/vnd.openxmlformats-officedocument.wordprocessingml.styles+xml")
	//d.ContentTypes.AddOverride("/word/glossary/settings.xml", "application/vnd.openxmlformats-officedocument.wordprocessingml.settings+xml")
	//d.ContentTypes.AddOverride("/word/glossary/webSettings.xml", "application/vnd.openxmlformats-officedocument.wordprocessingml.webSettings+xml")
	//d.ContentTypes.AddOverride("/word/glossary/fontTable.xml", "application/vnd.openxmlformats-officedocument.wordprocessingml.fontTable+xml")
...
func (d *Document) onNewRelationship(decMap *zippkg.DecodeMap, target, typ string, files []*zip.File, rel *relationships.Relationship, src zippkg.Target) error {
	...
	case gooxml.PackageType:
		var packageRef common.PackageRef
		for _, f := range files {
			...
			pkg := common.Package{Path: path}
			packageRef = common.NewPackageRef(&d.DocBase, d.docRels, pkg)
			d.Packages = append(d.Packages, packageRef)
			//files[i] = nil
		}
		rel.TargetAttr = strings.TrimPrefix(target, "word/")
	case gooxml.OLEObject:
		var oleObjectRef common.OLEObjectRef
		for _, f := range files {
			if f == nil || f.Name != target {
				continue
			}
			...
			oleObject := common.OLEObject{Path: path}
			oleObjectRef = common.NewOLEObjectRef(&d.DocBase, d.docRels, oleObject)
			d.OLEObjects = append(d.OLEObjects, oleObjectRef)
			//files[i] = nil
		}
		rel.TargetAttr = strings.TrimPrefix(target, "word/")
